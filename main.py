...
ADAFRUIT_IO_KEY = 'aio_raRI01kWY5twBLQWQI5iiQxzQOYp'
ADAFRUIT_IO_USERNAME = 'kido2k3'
FEED_KEY = ["iot-brightness", "iot-ai", "iot-humidity", "iot-led", "iot-pump", "iot-temperature"]
...
while True:
    if timer <= 0:
        ...
        print(f'Publishing {value} to iot-humidity.')
        client.publish('iot-humidity', value)
        value = random.randint(0, 2000)
        print(f'Publishing {value} to iot-brightness.')
        client.publish('iot-brightness', value)
        value = random.randint(15, 40)
        print(f'Publishing {value} to iot-temperature.')
        client.publish('iot-temperature', value)
        timer = 10
    timer = timer - 1
    time.sleep(1)
